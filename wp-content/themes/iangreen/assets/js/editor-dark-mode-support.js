/* global iangreenIsDarkMode, setTimeout */

// Check the color scheme preference and inject the classes if necessary.
if ( document.body.classList.contains( 'iangreen-supports-dark-theme' ) ) {
	iangreenDarkModeEditorInit();
}
 ...
 * @return {void}
 */
function iangreenDarkModeEditorInit( attempt ) {
	...
	if ( iangreenIsDarkMode() ) {
		if ( null === container ) {
			// Try again.
			if ( attempt < maxAttempts ) {
				setTimeout(
					function() {
						iangreenDarkModeEditorInit( attempt + 1 );
					},
